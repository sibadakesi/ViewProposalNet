...
def main():
    ...
    test_image_directory = '/media/xiong/19e5b012-b28b-4bc5-8a21-2a0237fec899/workspace/image_aadb/zhangding/test_727'
    image_format = 'jpg'
    image_list = glob.glob(os.path.join(test_image_directory, '*.{:s}'.format(image_format)))
    image_list.extend(glob.glob(os.path.join(test_image_directory, '*.png')))
    image_list.extend(glob.glob(os.path.join(test_image_directory, '*.JPG')))
    image_list.sort()
    result_save_directory = 'ProposalResults'
    #result_save_directory = 'liu'
    ...
    model_weight_path = 'model_params/ProposalNet/VPN'
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            for id, s_image_path in enumerate(image_list):
                    ...
                    image_annotation[s_image_name]['bboxes'] = bboxes_selected[0:pick_n] # x1 y1 x2 y2
